...
After successfully parsed a packet, we are able to acquire channels from it.

In a scan, there are 320 columns. However, we only load a few of them through each packet. Sometimes, we need to collect a full scan before processing all the data. `ce30_driver` provides a helper class call `scan` to deal with that. Let's do some modifications to the above snippet.
